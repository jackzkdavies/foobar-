...
# A STRIPS-style planner for the blocks world
    ...
    def __init__(self):
        ...
        """
        The added onlake formula is used to declare if a duck is on the lake
        The added notonlake formula is used to declare that a duck is not on th elake
        The added shoot formula is used to 'stack' the bullets on the duck
        """
        ...
        """
        Duckswim is used to set the duck to 'onlake'
        Duckfly is used to set the duck to 'notonlake'
        Shootduck is used to remove the bullets from on gun
        """
    ...
    """
    Used to set the an item on the lake
    """    
    def onlake(self):
        topG = self.top(self.goalStack)
    ...
    """
    Used to set an item as not on the lake
    """
    def notonlake(self):
        topG = self.top(self.goalStack)
        assert (topG[0] == "notonlake"),"expected notonlake"
        assert (len(topG) == 2 ),"expected 2 for notonlake"
        ## add the preconditions to the goalstack.
    ...
    """
    Used to shoot an item with the bullets in a gun
    """
    def shoot(self):
        topG = self.top(self.goalStack)
        assert (topG[0] == "shoot"),"expected shoot"
        assert (topG[2] == "bullets"),"expected shoot"
    ...
    """
    method used for shooting
    """  
    def shootduck(self,subgoal):
        #deletions
        x = self.getItemUnder(subgoal[2])
        self.worldStateRemove(["on","bullets", x])
    ...
    """
    used to add the item to the lake
    """    
    ...
    """
    Used to set the item to not being on the lake
    """
    ...
    def isonsomething(self, item):
        """
        Returns true if the item is on the table.
        False otherwise.
        """
        return (["on",item] in self.worldStack)
    ...
    def isnotonlake(self, item):
        """
        Return true if not is not on lake
        """
...
def runTests():
    ...
    print "\n\nRunning Test 3\n"
    ws = "((clear bullets), (clear gun), (ontable bullets), (ontable gun), (onlake duck),(on bullets gun), (armempty)) "
    gs1 = "((clear bullets),(armempty),(shoot duck bullets),(notonlake duck))"
